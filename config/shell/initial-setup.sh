...
if [[ ! -d '/.hephaestus-config' ]]; then
    ...
    echo 'Created directory /.hephaestus-config'
fi

touch '/.hephaestus-config/vagrant-core-folder.txt'
echo "${VAGRANT_CORE_FOLDER}" > '/.hephaestus-config/vagrant-core-folder.txt'
...
if [[ ! -f '/.hephaestus-config/initial-setup-apt-get-update' ]]; then
    ...
    touch '/.hephaestus-config/initial-setup-apt-get-update'
fi

# CentOS comes with tty enabled. RHEL has realized this is stupid, so we can
# also safely disable it in PuPHPet boxes.
if [[ ! -f '/.hephaestus-config/disable-tty' ]]; then
    perl -pi'~' -e 's@Defaults(\s+)requiretty@Defaults !requiretty@g' /etc/sudoers

    touch '/.hephaestus-config/disable-tty'
fi

# Digital Ocean seems to be missing iptables-persistent!
# See https://github.com/puphpet/puphpet/issues/1575
if [[ ! -f '/.hephaestus-config/iptables-persistent-installed' ]] && [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
    apt-get -y install iptables-persistent > /dev/null 2>&1

    touch '/.hephaestus-config/iptables-persistent-installed'
fi

if [[ ! -f '/.hephaestus-config/resolv-conf-changed' ]]; then
    echo "nameserver 8.8.8.8" > /etc/resolv.conf
    echo "nameserver 8.8.4.4" >> /etc/resolv.conf

    touch '/.hephaestus-config/resolv-conf-changed'
fi

if [[ -f '/.hephaestus-config/initial-setup-base-packages' ]]; then
    exit 0
fi
...
touch '/.hephaestus-config/initial-setup-base-packages'
